...
class CPL_Admin_Widgets_Tradingsg_DailyCollectionReport_Model extends CP_Common_Lib_WidgetModelAbstract
{
    ...
    function setSearchVar() {
        ...
        if ($start_date != '' && $end_date == '') {
  	        $searchVar->sqlSearchVar[] = "DATE_FORMAT(r.date, '%Y-%m-%d') >= '{$start_date}' AND DATE_FORMAT(r.date, '%Y-%m-%d') <= '{$current_date}'";
        } else if ($start_date == '' && $end_date != ''){
            $start_date = $year . '-' . $month . '-' . '01';
  	        $searchVar->sqlSearchVar[] = "DATE_FORMAT(r.date, '%Y-%m-%d') >= '{$start_date}' AND DATE_FORMAT(r.date, '%Y-%m-%d') <= '{$end_date}'";
            ...
            $searchVar->sqlSearchVar[] = "DATE_FORMAT(r.date, '%Y-%m-%d') >= '{$current_date}' AND DATE_FORMAT(r.date, '%Y-%m-%d') <= '{$current_date}'";
        }
        ...
        if ($cpCfg['cp.hasMultiUniqueSites']){
            ...
            $searchVar->sqlSearchVar[] = "r.site_id = '{$cpSiteIdSession}'";
        }

        $searchVar->groupBy   = "DATE_FORMAT(r.date, '%Y-%m-%d')";
    ...
    function getExportToExcel(){
        ...
        if ($cpCfg['cp.hasMultiUniqueSites']){
            $cpSiteIdSession = $fn->getSessionParam('cp_site_id');
            $appendSqlSite   = "  AND r.site_id = '{$cpSiteIdSession}'";
        }
