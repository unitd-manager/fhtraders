<?
class CPL_Admin_Modules_Tradingsg_DebitNote_View extends CP_Admin_Modules_Tradingsg_DebitNote_View
{
    ...
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            $urlInvoicePrint = "index.php?_topRm=finance&module=tradingsg_order&_spAction=printDebitRecord&invoice_code={$row['invoice_code']}&debit_note_id={$row['debit_note_id']}&invoice_type=normal&footer_logo=yes&showHTML=0";
        ...
        $rows .="
            ...
            {$listObj->getListPublishedImage($row['published'], $row['debit_note_id'])}
            {$listObj->getListRowEnd($row['debit_note_id'])}
            ";
        ...
        {$listObj->getListHeaderCell('Debit Code', 'invoice_code')}
        {$listObj->getListHeaderCell('Company Name', 'c.company_name')}
        {$listObj->getListHeaderCell('Debit Date', 'i.invoice_date')}
     ...
     */
     function getGenerateInvoiceForm() {
        ...
        while ($rowOI = $db->sql_fetchrow($resultOrderItem)) {
            ...
            JOIN invoice i ON (i.credit_note_id = it.credit_note_id)
     ...
     */
     function getEditInvoiceForm() {
        ...
        $invoiceRec = $fn->getRecordRowById('invoice', 'credit_note_id', $credit_note_id);
    ...
    function getPrintInvoice() {
        ...
        LEFT JOIN invoice i ON (i.credit_note_id = ini.credit_note_id)
