...
class CPL_Admin_Modules_Tradingsg_Supplier_Model extends CP_Common_Lib_ModuleModelAbstract
{
    ...
    function getGeneratePurchaseOrderFormSubmit() {
        ...
        $advanceAmountchk  = $fn->getPostParam('advanceAmountchk');
        if($advanceAmountchk == 1){
            $advance_amount  = $fn->getPostParam('advance_amount');            
        } else {
            $advance_amount  = 0;
        }
        ...
        $fa['advance_payment_used'] = $advance_amount;
    ...
    function getGeneratePurchaseOrderFormValidate() {
        ...
        if ($poCodes != ''){
            $SQL = "
                ...
                ,SUM(((pop.qty * pop.cost_price) * pop.gst) / 100) AS po_gst_amount
                FROM po_product pop 
                WHERE pop.purchase_order_id IN ({$po_code})
            ";
            $resultPaid = $db->sql_query($SQL);
            $rowPaid    = $db->sql_fetchrow($resultPaid);
            $total_po_amount = $rowPaid['po_amount'] + $rowPaid['po_gst_amount'];
    ...
    function getPopulatePOAmount() {
        ...
        $resultPaid = $db->sql_query($SQLPaid);
        $rowPaid    = $db->sql_fetchrow($resultPaid);
        $totalCost   = $rowPaid['po_amount'] + $rowPaid['GST_Total'] - $overall_discount;
    ...
    function getPopulatePOTotalAmount() {
        ...
        $resultPaid = $db->sql_query($SQLPaid);
        $rowPaid    = $db->sql_fetchrow($resultPaid);
        $totalCost   += $rowPaid['po_amount'] - $rowPaid['Discount_Total'] + $rowPaid['GST_Total'] - $overall_discount;
    ...
    function getGenerateAdvancePaymentFormSubmit() {
        ...
        if (!$this->getGenerateAdvancePaymentFormValidate()){
            return $validate->getErrorMessageXML();
        }
        ...
        $insertReceiptSQL     = $dbUtil->getInsertSQLStringFromArray($fa, 'advance_payment_history');
        $resultSQL            = $db->sql_query($insertReceiptSQL);
        $advance_payment_history_id  = $db->sql_nextid();
