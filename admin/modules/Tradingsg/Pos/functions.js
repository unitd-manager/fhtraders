...
cpm.tradingsg.pos = {
    init: function(){
        ...
        $(".addProduct input[name='product_title']").live("keydown", function (e) {
            ...
            if (keyCode == 13) {
                if(titleVal ==''){
                     //$("#orderItems>tr").parents('tr').prev('tr').find('input[name=qty]:first').focus();
       ...
       /*$('#thermalPrinterPrint').livequery('click', function (){
        ...
        $(".orderDatePOSHeader input[name='notes']").livequery('change', function(e){
            var notes  = $(this).val();
            var url = 'index.php?module=tradingsg_pos&_spAction=updateOrdernotes&showHTML=0';
            $.get(url, {notes:notes}, function(html){
            });
        });
