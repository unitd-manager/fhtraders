...
/**
    ...
    function getPrintPOtoPDF() {
        ...
        $address1        = $company['address_street'];
        $address_po_code = $company['address_po_code'];
        ...
        $tbl2 ='<table border="1" width="100%" cellpadding="6" style="font-size:15px;">
                    <tr>
                        <td width="64%" style="line-height:20px;"><br/>
                            ...
                            <span>&nbsp;&nbsp;&nbsp;&nbsp;'.$address_state.' - '.$address_po_code.'</span>
                        </td>
    ...
    function getPrintPOwithpricetoPDF() {
        ...
        $tbl3 ='<table border="1" width="100%" cellpadding="4" style="font-size:12px;">
                    <thead>
                        <tr>
        ...
        while($row = $db->sql_fetchrow($result)){
            ...
            $gstVal = ($row['total_price'] * $row['gst']) / 100;
            $total_price = $row['total_price'] + $gstVal;
            $tbl3 = $tbl3.'<tr>
                                <td width="7%" style="text-align:center;">'.$count.'</td>
                                <td width="38%">'.$row['title'].'</td>
                                <td width="10%" style="text-align:right;">'.$row['po_QTY'].'</td>
                                <td width="15%" style="text-align:right;">'.$row['cost_price'].'</td>
                                <td width="10%" style="text-align:center;">'.$row['gst'].'</td>
                                <td width="20%" style="text-align:right;">'.number_format($total_price, 2).'</td>
                            </tr>
                           ';

            $overallTotal += $total_price;
            $overallQty += $row['po_QTY'];
        ...
        $tbl3 = $tbl3.'<tr>
                            ...
                            <td style="text-align:right;">'.$overallQty.'</td>
        ...
        $tbl4 ='<table border="1" width="100%" cellpadding="4" style="font-size:13px;">
                    <thead>
                        <tr>
                            <th align="center" colspan="3" style="font-weight:bold;">PAYMENT DETAILS</th>
                        </tr>
        ...
        $numRows    = $db->sql_numrows($resultClient);
        $overallTotalPaid = 0;
        while($rowClient = $db->sql_fetchrow($resultClient)){
            $date = $fn->getCPDate($rowClient['date'], 'd-m-Y');
            $tbl4 = $tbl4.'<tr>
                                <td>'.$date.'</td>
                                <td>'.$rowClient['mode_of_payment'].'</td>
                                <td style="text-align:right;">'.$rowClient['amount'].'</td>
                            </tr>
                           ';
            $overallTotalPaid += $rowClient['amount'];
        }
        $balanceAmount = round($overallTotal - $overallTotalPaid);

        $tbl4 = $tbl4.'
                        <tr>
                            <td colspan="2" style="text-align:right;">Amount Paid</td>
                            <td style="text-align:right;">'.number_format($overallTotalPaid, 2).'</td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align:right;">Balance Amount</td>
                            <td style="text-align:right;">'.number_format($balanceAmount, 2).'</td>
                        </tr>
                        ';
    ...
    function getPrintPOtoExcel() {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $address1        = $row['address_street'];
            $address_po_code        = $row['address_po_code'];
        ...
        foreach($poProductId AS $po_product_id){
            if($po_product_id != ''){
                ...
                if($PoInv['actual_stock'] > 0){
                    $stock = $PoInv['actual_stock'] + $PoProd['qty'];
                } else {
                    $stock = $PoProd['qty'];
                }
