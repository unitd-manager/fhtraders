<?
class CPL_Admin_Lib_PhpExcelImportWrapper extends CP_Common_Lib_PhpExcelImportWrapper
{
...
//            }
    ...
    function importData($cfg) {
        ...
        if(!is_array($rec) || count($rec) == 0){
            ...
            $text .= "
            INSERT INTO product (part_number,title,unit,price_from_supplier,price,product_group_id,published,member_only,creation_date,created_by)
            VALUES ('{$fa['part_number']}','{$fa['title']}','{$fa['unit']}',{$fa['price_from_supplier']},{$fa['price']},{$fa['product_group_id']},{$fa['published']},1,'{$date}','Administrator');
            ";
            ...
            $date  = date('Y-m-d H:i:s');
            $text .= "
            UPDATE product SET price_from_supplier='{$fa['price_from_supplier']}', price='{$fa['price']}', member_only=1, modification_date='{$date}', modified_by='Administrator'
